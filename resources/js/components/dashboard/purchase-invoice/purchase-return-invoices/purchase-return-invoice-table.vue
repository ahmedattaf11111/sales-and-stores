<template>
  <div class="purchase-return-invoice-container">
    <DeleteConfirmation
    ...
    <div class="header">
      ...
      <div class="title">
        ...
        /
        <span>{{ $t("GENERAL_PURCHASE_RETURN_INVOICES") }}</span>
      </div>
    </div>
    <div class="px-4">
      <div class="table-container">
          ...
          <div class="actions">
            <span data-toggle="tooltip" data-placement="top" :title="$t('ADD')">
              <button
                @click="onAddClicked()"
                data-toggle="modal"
                data-target="#purchaseReturnInvoiceFormModal"
        ...
        <div class="mt-1">
          <paginate
            ...
            :clickHandler="getPurchaseReturnInvoices"
            :prevText="$t('PREV')"
            :nextText="$t('NEXT')"
          >
          </paginate>
...
import PurchaseReturnInvoiceForm from "./purchase-return-invoice-form.vue";
import ApproveInvoiceForm from "./approve-invoice-form.vue";
import purchaseInvoiceStore from "./purchase-return-invoice-store";
...
export default {
  ...
  setup() {
    ...
    function routeToItemsPage(purchaseInvoiceId) {
      router.push(`/purchase-return-invoice-items/${purchaseInvoiceId}`);
    }
    ...
    function getPurchaseReturnInvoices() {
    ...
    function getSuppliers() {
      purchaseReturnInvoiceClient.getSuppliers().then((response) => {
        data.suppliers = response.data;
      });
    }
    function getStores() {
      purchaseReturnInvoiceClient.getStores().then((response) => {
...
<style lang="scss">
.purchase-return-invoice-container {
  padding-bottom: 50px;
  .header {
    * {
