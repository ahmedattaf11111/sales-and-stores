...
export default {
  ...
  setup() {
    ...
    function onCreated(event) {
      data.customers.unshift(event);
    }
    function onUpdated(event) {
      data.customers[data.selectedCustomerIndex] = event;
      data.selectedCustomer = null;
    }
    ...
    function onItemInfoClicked(customer) {
      let infos = [
        { header: "CREATED_AT", text: customer.account.created_at, textDateTime: true },
        { header: "BY", text: customer.account.added_by.name },
      ];
      if (customer.account.updated_by) {
        infos = infos.concat([
          { header: "UPDATED_AT", text: customer.account.updated_at, textDateTime: true },
          { header: "BY", text: customer.account.updated_by.name },
        ]);
      }
      infos=infos.concat([
          { header: "ACCOUNT_NUMBER", text: customer.account.id },
          { header: "CUSTOMER_NUMBER", text: customer.id },
      ])
      data.infos = infos;
    }
    //Commons
